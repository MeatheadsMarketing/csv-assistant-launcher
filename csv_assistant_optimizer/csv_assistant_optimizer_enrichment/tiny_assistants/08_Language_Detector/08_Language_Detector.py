...
def language_detector(df: pd.DataFrame, **kwargs) -> pd.DataFrame:
    """
    Identifies language of free text columns.
    ...
    try:
        st.info("Running assistant: Language Detector")
        # TODO: Implement enrichment logic here
        pass
    except Exception as e:
        st.error(f"Error in assistant 'Language Detector': {e}")

    return df_enriched

def run_ui():
    st.title("Language Detector")
    st.write("Identifies language of free text columns.")
    ...
    if uploaded_file:
        ...
        result_df = language_detector(df)
